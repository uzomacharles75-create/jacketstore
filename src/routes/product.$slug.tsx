      ...
      <main className="flex-1">
        ...
        <section className="container-x grid lg:grid-cols-2 gap-10 pb-16">
          ...
          <div>
            ...
            <p className="mt-4 text-3xl font-bold text-secondary">P {product.price.toLocaleString()}</p>
            ...
            <div className="mt-6 flex items-center gap-2 text-sm text-bush">
              <Check className="h-4 w-4" /> In stock — ready to ship
            </div>

            <a
              href={waLink(`Hello, I want to order: ${product.name} (P ${product.price}). My size: ___, color: ___.`)}
              target="_blank"
        ...
        <section className="container-x py-16 border-t border-border">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {fallbackRelated.map((p) => <ProductCard key={p.slug} product={p} />)}
          </div>
        </section>
      </main>
      <Footer />
